...
# ---------------------------------------------------------------------------
# AR5 land use proximity scoring
# ---------------------------------------------------------------------------
...
def score_ar5_land_use(
    ...
    lakes["ar5_land_use_score"] = gpd.pd.concat(
        [ind_scores, res_scores], axis=1
    ).min(axis=1).astype(int)
    lakes["ar5_land_use_level"] = lakes["ar5_land_use_score"].map(LEVEL_NAMES)
    return lakes
...
_DIMENSION_COLUMNS = ["cabin_density_score", "accessibility_score", "ar5_land_use_score"]
...
def process_scoring(
    ...
    Steps:
      ...
      3. AR5 land use proximity (optional, enabled by default)
      4. Compute composite tentability score (worst dimension wins)
    """
    # --- Cabin density ---
    ...
    if config.ar5_land_use.enabled:
        ...
        industrial_polygons, residential_polygons = fetch_ar5_land_use_polygons(
            gdb_paths, bbox
        )
        lakes = score_ar5_land_use(
            lakes, industrial_polygons, residential_polygons, config.ar5_land_use
        )
        _print_distribution("AR5 land use", lakes, "ar5_land_use_score")
    else:
