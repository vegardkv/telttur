...
| File | Purpose |
|------|---------|
| `config.yaml` | Default local/dev config (small Oslo-area bbox) |
| `config_mini.yaml` | Tiny bbox — fastest full-pipeline smoke test (~10 s with warm caches) |
| `config_akershus.yaml` | Regional map (Akershus); generated by CI on pushes to main, embeds single-file HTML |
| `config_innlandet.yaml` | Regional performance test (Innlandet) |
| `config_norway.yaml` | Full-country map (5 ha min lake area, N50 AR5 source) |
| `config_debug_blefjell.yaml` | Road-classification debug map (`debug_map: true`, tight bbox) |
